...
int main() {
    const int num_atoms = 20;
    const float radius = 100.0f;

    cout << "[headless] hydrogen atom model output\n";
...
struct Engine {
    ...
    Engine () {
        // --- Init GLFW ---
        if (!glfwInit()) {
            cerr << "failed to init glfw, falling back to text mode\n";
            headless = true;
            return;
        }
        ...
        if (!window) {
            cerr << "failed to create window, falling back to text mode\n";
            glfwTerminate();
            headless = true;
            return;
        }
...
static void runHeadlessSimulation(vector<Atom>& atoms, vector<Wave>& waves) {
    cout << "[headless] running atom model in text mode\n";
    cout << "[headless] atoms=" << atoms.size() << " waves=" << waves.size() << "\n";

    for (int step = 0; step < 5; ++step) {
        cout << "[headless] step " << step << "\n";
        for (size_t i = 0; i < atoms.size(); ++i) {
            const Atom& atom = atoms[i];
            cout << "  atom[" << i << "] pos=(" << atom.pos.x << ", " << atom.pos.y << ")\n";
        }

        for (size_t i = 0; i < waves.size(); ++i) {
            const Wave& wave = waves[i];
            cout << "  wave[" << i << "] energy=" << wave.energy << " sigma=" << wave.sigma << "\n";
        }
    }
}
...
int main () {
    ...
    if (engine.headless || !engine.window) {
        ...
        runHeadlessSimulation(atoms, waves);
        return 0;
    }
